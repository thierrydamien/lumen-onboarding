...
function procTopics(t) {
  const s = [];
  // A topic suggestion can arrive in two shapes, and BOTH must be parsed into cards
  // AND removed from the visible text — otherwise the raw marker leaks into the
  // client's message (the live-build bug where a client saw TOPIC_SUGGESTION{...}).
  //  (a) JSON form: TOPIC_SUGGESTION{...} — the model emits this naturally because
  //      every other structured field in the prompt is JSON; it mirrors the
  //      %%TOPICS%% schema (group/name/keywords/urls/hashtags/comments). Matched
  //      across newlines; flat objects only (keywords are boolean text, no braces).
  //  (b) Legacy pipe form: TOPIC_SUGGESTION|name|keywords|rationale — one per line.
  let text = t.replace(/TOPIC_SUGGESTION\s*(\{[^{}]*\})/g, (_m, j) => {
    try {
    ...
    } catch { /* malformed JSON — still dropped below so nothing leaks */ }
    return "";
  });
  const k = [];
...
// as a chat turn: cap it small so the round-trip stays fast (a big raw dump was
// what timed out the serverless call on the live build) and the assistant is told
// to use it to pre-fill + confirm, not regurgitate it.
...
function OnboardingApp({ seed, seedId, onBriefSent, onSeeProserv }) {
  ...
  const sendAttachment = useCallback(async (file) => {
    if (!file || loading || attaching) return;
    setAttachNote(null);
    setAttaching(true);
    try {
      ...
      {showPanel && started && <div style={{position:"fixed",top:56,right:0,bottom:0,width:mob?"100%":320,background:C.card,borderLeft:`1px solid ${C.border}`,zIndex:500,overflowY:"auto",padding:"16px 18px",boxShadow:sideCol?"none":"-4px 0 16px rgba(0,0,0,0.08)"}}>
        ...
        {(() => {
          ...
          return <>
            ...
            {captured.length===0 && <div style={{fontSize:12,color:C.muted,lineHeight:1.6}}>Your answers will appear here as we go.</div>}
            {pending.length>0 && <div style={{marginTop:captured.length?6:12,paddingTop:captured.length?10:0,borderTop:captured.length?`1px solid ${C.border}`:"none",fontSize:11,color:C.muted,lineHeight:1.5}}>{pending.length} more {pending.length===1?"detail":"details"} will fill in as you chat.</div>}
          </>;
        })()}
      </div>}
      ...
      <div ref={msgRef} style={{flex:1,overflowY:"auto",padding:"24px 16px",maxWidth:760,width:"100%",margin:"0 auto",alignSelf:"center",transform:sideCol&&showPanel&&started?"translateX(-160px)":"none",transition:"transform 0.25s ease"}}>
        ...
        {retryMsg && !loading && <div style={{display:"flex",justifyContent:"flex-start",marginBottom:18,animation:"slideUpFade 0.3s ease-out forwards"}}>
          ...
          <div style={{background:dark?"#3a2f1a":"#fffbeb",border:`1px solid ${dark?"#5c4a24":"#fde68a"}`,borderRadius:12,padding:"12px 16px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
            ...
            <span style={{fontSize:13,color:dark?"#e8d9b5":"#92400e"}}>That didn't go through. Tap Try again to resend.</span>
            <button onClick={()=>sendToAPI(retryMsg,true)} style={{background:A,color:"white",border:"none",borderRadius:8,padding:"7px 16px",fontSize:12,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",boxShadow:T.shadow.glow}}>Try again</button>
          </div>
        </div>}
        ...
        <div style={{maxWidth:760,margin:"0 auto",transform:sideCol&&showPanel&&started?"translateX(-160px)":"none",transition:"transform 0.25s ease"}}>
          <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
            ...
            <textarea ref={taRef} value={input}
              ...
              aria-label="Type your reply" placeholder={last?.role==="assistant"&&(last.widgets||[]).some(w=>!wState[w]?.submitted)?"Answer above — or just type it here":"Type your reply…"} rows={1}
