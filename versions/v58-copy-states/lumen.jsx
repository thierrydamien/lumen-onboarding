  ...
  return <div style={{marginTop:8}}>
    ...
    {sel.length>0 && <div style={{background:"#f8f9fa",border:"1px solid #e2e8f0",borderRadius:10,padding:"10px 12px",marginBottom:10}}>
      ...
      {sel.map((o,i) => <div key={o} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderTop:i>0?"1px solid #eef1f5":"none"}}>
        ...
        <button onClick={()=>move(i,-1)} disabled={i===0} aria-label={`Move ${o} up`} style={{background:"transparent",border:"1px solid #e2e8f0",borderRadius:8,width:32,height:32,cursor:i===0?"default":"pointer",color:i===0?"#cbd5e1":"#64748b",fontSize:12,lineHeight:1,flexShrink:0}}>▲</button>
        <button onClick={()=>move(i,1)} disabled={i===sel.length-1} aria-label={`Move ${o} down`} style={{background:"transparent",border:"1px solid #e2e8f0",borderRadius:8,width:32,height:32,cursor:i===sel.length-1?"default":"pointer",color:i===sel.length-1?"#cbd5e1":"#64748b",fontSize:12,lineHeight:1,flexShrink:0}}>▼</button>
        <button onClick={()=>toggle(o)} aria-label={`Remove ${o}`} style={{background:"transparent",border:"1px solid transparent",borderRadius:8,width:32,height:32,color:"#ef4444",cursor:"pointer",fontSize:13,flexShrink:0}}>✕</button>
      </div>)}
    </div>}
      ...
      <div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>
        ...
        <Section title="Where we'll look" badge={chans.length} defaultOpen={false}>
          ...
          {chans.map((ch,i) => <div key={ch.id} style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8,alignItems:"center"}}>
            {["author","type","url","owned"].map(k => <input key={k} value={ch[k]||""} placeholder={k} onChange={e=>setChans(cs=>cs.map((x,j)=>j===i?{...x,[k]:e.target.value}:x))} style={{flex:"1 1 140px",minWidth:0,border:"1px solid #e2e8f0",borderRadius:6,padding:"6px 8px",fontSize:11,outline:"none"}}/>)}
        ...
        <Section title="Reports and alerts" badge={reports.length+alerts.length} defaultOpen={false}>
          ...
          {reports.map((r,i) => <div key={r.id} style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8,alignItems:"center"}}>
            {["name","objective","details","comments"].map(k => <input key={k} value={r[k]||""} placeholder={k} onChange={e=>setReports(rs=>rs.map((x,j)=>j===i?{...x,[k]:e.target.value}:x))} style={{flex:"1 1 140px",minWidth:0,border:"1px solid #e2e8f0",borderRadius:6,padding:"6px 8px",fontSize:11,outline:"none"}}/>)}
          ...
          {alerts.map((a,i) => <div key={a.id} style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8,alignItems:"center"}}>
            {["name","type","details","comments"].map(k => <input key={k} value={a[k]||""} placeholder={k} onChange={e=>setAlerts(as=>as.map((x,j)=>j===i?{...x,[k]:e.target.value}:x))} style={{flex:"1 1 140px",minWidth:0,border:"1px solid #e2e8f0",borderRadius:6,padding:"6px 8px",fontSize:11,outline:"none"}}/>)}
    ...
    <div style={{display:"flex",justifyContent:"center",marginBottom:24,animation:"slideUpFade 0.5s ease-out forwards"}}>
      <div style={{background:`linear-gradient(135deg,${P}15,${P}08)`,border:`1.5px solid ${P}`,borderRadius:14,padding:"20px 28px",textAlign:"center",maxWidth:460}}>
        {/* Pre-send this card is a CALL TO ACTION, not a finish line: no celebration
            emoji/title until the brief has actually reached the Lumen team, so a
            skimming client can't read "100% + party" as done and close the tab. */}
        <div style={{fontSize:20,marginBottom:8}}>{sent?"\u2705":"\ud83d\udce8"}</div>
        <div style={{fontWeight:700,fontSize:15,color:C.text,marginBottom:6}}>{sent?"Brief sent to your Lumen team":"One last step: send your brief"}</div>
