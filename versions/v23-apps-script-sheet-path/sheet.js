...
export default async (req) => {
  ...
  // real Google account, so the Sheet is created in that account's folder on its
  // quota — no service account / OAuth / delegation needed. URL + secret are
  // server-side env; the browser never sees them.
  const appsUrl = process.env.APPS_SCRIPT_WEBAPP_URL;
  if (appsUrl) {
    try {
      const r = await fetch(appsUrl, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ secret: process.env.APPS_SCRIPT_SECRET || "", xlsxBase64, filename: name, clientEmail: clientEmail || "", company: company || "" }),
      });
