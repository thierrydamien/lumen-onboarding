...
function ExportModal({ cdata, wState, messages, onClose, onExport, onSend, sending, sendErr, sent, sheetLink, uiLang }) {
  ...
  // Reopened-with-data counts as captured, matching gwp in the panel. Gating on
  // `submitted` alone meant pressing "Edit" on a widget removed that answer from the
  // brief, the Sheet and the dashboard — with the client never told, and no Cancel to
  // put it back. Interruption is the normal case here, so the half-finished edit is a
  // path real clients take.
  const gw = type => { const es=Object.entries(wState||{}).filter(([k,v])=>k.endsWith(`-${type}`)&&(v===true||v?.submitted||v?.data!==undefined)).sort((a,b)=>(parseInt(a[0])||0)-(parseInt(b[0])||0)); const d=es.length?es[es.length-1][1].data:null; return d==="__skip__"?null:d; };
...
function OnboardingApp({ seed, seedId, seedError, seedExpired, onBriefSent, onSeeProserv }) {
  ...
  //
  // The measurement has to be taken as the client SCROLLS, not after the new turn
  // lands. Measuring inside the [messages] effect reads the DOM once React has
  // already appended the reply, so `scrollHeight - scrollTop - clientHeight` is the
  // height of the new content itself — over 200 for every widget turn and most prose
  // turns — and the guard concluded the client had scrolled away when they had not
  // moved at all. Measured at 1280x720 after two ordinary turns: scrollTop 0,
  // 1,152px unseen, and the widget's Confirm button at y=1723 in a 720px viewport,
  // while the composer told the client to "Answer above". They answered whatever was
  // visible and never saw the question.
  //
  // Intent is tracked from wheel / touch / key events, NOT from `scroll`. A plain
  // `scroll` listener looks right and is not: this component scrolls the transcript
  // itself on every send, and a smooth scroll emits a stream of `scroll` events whose
  // mid-animation position is far from the bottom. The listener then records "the
  // client scrolled away" about the app's own animation, and if the reply lands before
  // that animation finishes the new turn is never followed. Measured with
  // scrollIntoView instrumented: the send-phase call fired, the reply-phase call did
  // not, which is the original bug wearing a different hat.
  //
  // wheel, touchmove and the scroll keys only ever come from the client, so they
  // cannot be polluted by our own scrolling. Dragging the scrollbar is the one gap;
  // it fails safe, because the next turn simply follows.
  const nearBottomRef = useRef(true);
  useEffect(() => {
    const el = msgRef.current;
    if (!el) return;                                   // transcript not mounted yet
    const measure = () => { nearBottomRef.current = el.scrollHeight - el.scrollTop - el.clientHeight < 200; };
    const onKey = e => { if (["PageUp","PageDown","Home","End","ArrowUp","ArrowDown"," "].includes(e.key)) requestAnimationFrame(measure); };
    el.addEventListener("wheel", measure, { passive: true });
    el.addEventListener("touchmove", measure, { passive: true });
    el.addEventListener("keydown", onKey);
    return () => {
      el.removeEventListener("wheel", measure);
      el.removeEventListener("touchmove", measure);
      el.removeEventListener("keydown", onKey);
    };
  }, [checked, started]);                              // re-bind once the real tree exists
  useEffect(() => {
    if (nearBottomRef.current || loading) botRef.current?.scrollIntoView({behavior:REDUCE_MOTION?"auto":"smooth"});
  }, [messages, loading]);
  ...
  const renderWidget = useCallback((type, mi, topicSuggestions) => {
    ...
    // A reopened widget (Edit pressed, previous answer still in hand) needs a way
    // back that is not Confirm-again or Skip. Skip DESTROYS the answer, so without
    // this the only non-destructive exit was to re-submit — and a client who opened
    // Edit to look at something had no way to say "never mind". One control here
    // covers all five widget types rather than five separate footers.
    const reopened = !sub && ws && ws !== true && ws.data !== undefined;
    return <div aria-busy={loading ? "true" : undefined}
      ...
      {reopened && <div style={{display:"flex",justifyContent:"flex-end",margin:"0 0 6px"}}>
        <button onClick={()=>setWState(p=>({...p,[key]:{...p[key],submitted:true}}))}
          style={{background:"transparent",border:"none",color:C.muted,fontSize:11,textDecoration:"underline",cursor:"pointer",padding:"2px 4px",minHeight:24}}>
          {WL("cancelEdit",uiLang)}
        </button>
      </div>}
  ...
  // Counts a REOPENED widget too. "Edit" sets submitted:false but keeps data, so
  // gating on `submitted` alone dropped the answer out of "Captured so far" the
  // instant the client pressed Edit — before they had changed anything — and out of
  // the delivered brief with it (see the matching guard in ExportModal's gw). A
  // client who opens Edit and is then interrupted loses the answer silently, which
  // is the one thing this app must never do. Presence of `data` is the real test;
  // `submitted` only says whether they are currently looking at it.
  const gwp = type => { const es=Object.entries(wState).filter(([k,v])=>k.endsWith(`-${type}`)&&(v===true||v?.submitted||v?.data!==undefined)).sort((a,b)=>(parseInt(a[0])||0)-(parseInt(b[0])||0)); return es.length?es[es.length-1][1].data:null; };
