...
function briefSystemBlock(brief) {
  return "SALES BRIEF for this seeded session: the client's own team supplied these facts up front, so you ALREADY know them. Unlike the confidential consultant notes, you MAY reference these openly when relevant. Do NOT ask for them cold as if you knew nothing, and do NOT recite them all back at once. OPENING: greet, confirm ONLY the company name in one short sentence, then ask ONE question (the goal question at STEP 1.5). Keep the whole opener to about two sentences plus that one question, and NEVER list the brief's contents (competitors, markets, channels, current tools) in the opening message. Bring each known fact in LATER, one at a time, only when it is relevant to the question at hand (for example when discussing competitors: \"Your team mentioned Nike and Adidas, are those the main ones?\"). If a value looks off or is missing, ask about just that one thing. Treat every value as a starting point to verify, not settled fact, and keep applying your normal quality standard. Brief: " + brief;
}
